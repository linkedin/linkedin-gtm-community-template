...
[
  ...
  },
  {
    ...
    "simpleValueType": true,
    "help": "(Optional) Enter a numeric ID if available for event-specific conversions created in LinkedIn’s Campaign manager app.",
...
- name: Test sendPixel URL when all fields present
  code: |-
    ...
    mock('sendPixel', (url, onSuccess, onFailure) => {
      assertThat(url).contains('https://px.ads.linkedin.com/collect/?pid=123&conversionId=12576358&url=google.com&v=2&fmt=js&time=');
      onSuccess();
    });
...
- name: Test sendPixel URL when missing fields
  code: |-
    ...
    mock('sendPixel', (url, onSuccess, onFailure) => {
      assertThat(url).contains('https://px.ads.linkedin.com/collect/?pid=123&url=' + getUrl() + '&v=2&fmt=js&time=');
      onSuccess();
    });
